# ADR-0051: タブ切替に方向つき View Transition を掛ける

- **状態**: 採用
- **日付**: 2026-08-09
- **カテゴリ**: ux
- **関連**: [ADR-0002](../architecture/0002-no-router-tab-enum-signal.md), [ADR-0003](../architecture/0003-wasm-target-scoped-dependencies.md), [ADR-0049](../architecture/0049-browser-support-policy.md)

## 背景

タブは 記録 / 推移 / 種目 の 3 つで、enum の signal を差し替えるだけで切り替わる（[ADR-0002](../architecture/0002-no-router-tab-enum-signal.md)）。ルーターが無いぶん実装は短いが、**画面が瞬時に入れ替わるので前後関係が絵に出ない**。3 枚が横に並んでいるという構造は、タブバーの並びからしか読めなかった。
...
**`wasm-bindgen-futures` が直接依存に増える。** 既に leptos 経由で依存グラフには居たが、[ADR-0003](../architecture/0003-wasm-target-scoped-dependencies.md) の方針に従って明示した。wasm32 の target 別 dependencies なので `cargo test` のホストビルドには影響しない。
...
**`cross-document-transitions` を使う。** ルーターが無く 1 ドキュメントで完結しているので前提が成立しない（[ADR-0002](../architecture/0002-no-router-tab-enum-signal.md)）。
