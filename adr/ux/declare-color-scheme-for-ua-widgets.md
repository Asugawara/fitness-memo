# `color-scheme` を宣言し、クラスなしの `<button>` を作らない
...
「コピー」は書き出しシートの「うまくいかないとき」の中にある。共有シートもダウンロードも駄目だった端末に残る**最後の逃げ道**（[書き出しは共有シートを主経路にし、iOS では `<a download>` を使わない](../storage/share-sheet-over-download.md)）なので、いちばん困っているときに押せないのが困る。
...
他のボタンは全部 `primary` / `secondary` / `opt` / `link-btn` / `icon-btn` / `pick` / `seg-btn` / `tab-btn` / `menu-cand` / `cal-day` のどれかを持っていて無事だった。**見つかった 1 箇所は、規約から外れた 2 箇所のうちの 1 つ**だった。
...
**`color-scheme` は既存の方針と噛み合う。** このリポジトリは「ネイティブのコントロールをできるだけそのまま使う」で通してきた（`.target-select` の `appearance: auto`、`input[type="text"]` を使う [数値入力に `type="number"` を使わない](text-input-not-number.md)）。`color-scheme` はその選択の**前提条件**で、宣言しないままネイティブに任せるのは中途半端だった。
