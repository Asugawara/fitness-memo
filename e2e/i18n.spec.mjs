...
test('★ 言語を切り替えても種目名は変わらない（自分で付けた名前として扱う）', async ({ page }) => {
  await boot(page);

  // 英語で入ったプリセットを 1 つ改名する
  ...
  const chest = await openGroup(page, 'Chest');
  // 改名したものも、していないものも英語のまま
  await expect(chest.getByTestId('exercise-name').first()).toHaveText('My Bench');
  await expect(chest.getByTestId('exercise-name').nth(1)).toHaveText('Dumbbell Press');
  // 部位名も変わらない（「胸」に戻らない）
  await expect(chest.getByTestId('group-name')).toHaveText('Chest');
});
