...
- Switching languages does **not** rename exercises or muscle groups you already have. Those names are treated as data you own — renaming is a legitimate thing to have done, so the app never overwrites them ([Seed preset names once, in the launch language, and treat them as user data](adr/storage/preset-names-are-user-data-seeded-once.md)). Change them one at a time under Settings → Exercises.
