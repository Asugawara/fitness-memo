...
> **追記（マニュアル節の時点）— 除外の基準を改訂した。**
> 下の「結果（トレードオフ）」にある **「以後この `find` の除外に載せてよいのは
> アプリが一度も参照しないファイルだけ」は、この時点で狭すぎることが分かった。**
> [マニュアルの図は `public/` に配信するスクリーンショットにし、`<img>` で参照する](../architecture/manual-figures-as-served-screenshots.md)
> の `public/manual/**` は**アプリが `<img>` で参照する初のシェル外配信物**で、
> 旧基準に正面から当たる。
>
> **新基準**: 除外に載せてよいのは「アプリが一度も参照しないもの」、または
> 「**参照するが、欠けても本文だけで完結する補助資産**」。後者は次の 5 条件を
> **全部**満たすものに限る（主観判定にすると次に触る人が誤用するので、
> 検査できる条件に落とした。実装は `scripts/stamp-sw.sh` の `shell_files()` に
> 同じ 5 条件がコメントとして入っている）。
>
...
>
> 条件 5 が新基準の中核である。旧基準が成立していたのは `og.png` について
> 「アプリが参照しない」を機械で確かめられたからで、参照するものを載せる以上、
> **欠けたときに何が起きるかを機械で押さえる**代替が要る
> （`e2e/manual.spec.mjs` の「オフラインで開くと本文は読めるが図は欠け、断りが伝わる」）。
>
> **`og.png` の扱いは 1 バイトも変わらない**（前者に該当したまま）。除外語が
> `og.png` の 1 語から `og.png` / `./manual/*` / dotfile の 3 語に増えたことは
