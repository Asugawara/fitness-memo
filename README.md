...
- **最後のトレーニングからの経過** — 全体と部位ごとに表示する。**日数はローカル暦の日差**で、日を跨いだら「昨日 / 3日前」、同じ日のうちだけ「45分 / 12時間」の時刻粒度になる。経過時間を 24 時間で割ると繰り上がりがトレーニング時刻の 24 時間後に来てしまい、昨夜の記録が翌朝に「今日」と出る（[ADR-0054](adr/data-model/0054-elapsed-in-local-calendar-days.md)）。
- **体重と体調メモ** — 日ごとに 1 行で記録できる。トレーニングしていない日でも記録でき、その日も上のグラフに乗る。
- **他アプリからの移行** — エクスポート機能を持たないアプリからでも移せる。前のアプリの画面をスクショし、iOS の「テキスト認識表示」で起こした文字を貼り付けると、日付・種目・セットを読み取って**足すだけ**で取り込む（[ADR-0055](adr/ux/0055-migrate-by-ocr-paste.md)）。英語の種目名はプリセットの日本語名に寄せる。読み取れなかった行は全文を画面に出す。
...
- **他アプリから** — 前のアプリのスクショを iOS の「テキスト認識表示」で起こして貼り付ける。読み取った内容と、知らない種目の行き先（既存の部位）と、読み取れなかった行を出してから確認へ進む。**取り込みは常に「足すだけ」**（[ADR-0056](adr/data-model/0056-text-import-is-merge-only.md)）
...
- 他アプリのデータをアプリ側から自動で読むことはできない（iOS のサンドボックス）。ショートカットも HealthKit も Web Share Target も経路にならないので、移行はスクショの文字起こしを貼り付ける形だけを持つ。狙いは**種目リストと直近の重量**で、数年分の履歴を再現する用途には応えない（[ADR-0055](adr/ux/0055-migrate-by-ocr-paste.md)）。
