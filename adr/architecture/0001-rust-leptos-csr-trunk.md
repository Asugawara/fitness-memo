# ADR-0001: Rust + Leptos (CSR) + trunk を採用する
...
- 型安全なドメインモデル（`Kind` / `Elapsed` / `Db`）が、この後 ADR-0006・0007 で扱う「静かに壊れる」バグ群を型で防いでくれる。
- WASM のバンドルサイズは gzip で 300〜500KB 程度になるが、**オフライン PWA なので初回ダウンロードのみ**。Service Worker がキャッシュしたあとは影響しない。
- グラフは自前 SVG（ADR-0004）、日付は chrono、永続化は localStorage で、いずれも Rust から素直に書ける。
...
- ホストターゲットでのコンパイル可否という前提が生まれたが、ADR-0003 の target 分離で解消した。
