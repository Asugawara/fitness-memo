...
- **日付**: 2026-08-08
- **カテゴリ**: pwa
- **関連**: [数値入力に `type="number"` を使わない](../ux/text-input-not-number.md)
...
- **タイマーは leptos の `set_timeout_with_handle` / `TimeoutHandle::clear` を使う**（0.8.20 に実在）。`wasm-bindgen` の `Closure` を自前で扱う必要がなくなり、`web-sys` の feature も増えない（[UI 依存を wasm32 の target 別 dependencies に置く](../architecture/wasm-target-scoped-dependencies.md)）。
...
- **DevTools では検証できない。** レスポンシブモードにキーボードが無いので、`.kb-open` が付くこと自体は E2E で確認できるが、「タブバーが入力欄を塞がない」は**実機でしか見えない**。キーボードを出した状態でボトムタブが入力欄を塞がないことは、実機で確認するしかない。
