...
> この状態になっても、Safari のタブ側で「データの書き出し」→ PWA 側で「読み込み」を通せば移行できる（設定タブの「データの書き出し / 読み込み」）。
...
設定タブの「データの書き出し / 読み込み」から:

- **書き出し** — 1 タップで **TSV**（`fitness-memo-YYYYMMDD-HHMM.tsv`）を出す。iPhone では共有シートが開くので、**「ファイルに保存」→ iCloud Drive / Google Drive** を選ぶと機種を替えても残る。**Google スプレッドシートでそのまま開ける**（1 セット 1 行の表）
- **読み込み** — 書き出したファイルを選ぶ。実行前に「現在」と「取り込み後」の件数、そして何が増えるかを出し、**取り込む直前に今のデータを自動で退避**してから適用する（直後なら「元に戻す」で戻せる）
