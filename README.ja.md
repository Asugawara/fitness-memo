...
- ラベルは種目ごとの定義で、種目をまたいで共有できない。1 種目 6 本まで・1 つ 12 文字まで・作成順（並べ替えも色も無い）。削除は不可逆で、**同じ名前で作り直しても過去の記録はそのラベルには戻らない**（参照が ID なので）。記録そのものには触らないので「指定なし」でそのまま見える。定義したが 1 度も使っていないラベルは TSV に載らない（JSON バックアップには入る。打ち直せば完全に戻る）。**推移タブはラベルを見ない。**
