...
> **追記（マニュアル節の時点）**
> - **この決定は無変更で有効。** ヘルプの 3 枚は今も `assets/help/*.svg` +
>   `include_str!` + `inner_html` のままで、1 行も差し替えていない
> - **「検討した代替案」で実機スクリーンショットを却下した 3 つの理由は、
>   この被写体（iOS のブラウザクローム）に対しては今も有効。** 共有シートには
>   実在の連絡先が写るし、模式図でないとテーマに追従できない
> - **ただし「後から採る余地は残した」を、別の被写体に対して行使した。**
>   [マニュアルの図は `public/` に配信するスクリーンショットにし、`<img>` で参照する](manual-figures-as-served-screenshots.md)
>   が **アプリ自身の画面**を実スクショで撮り、`public/manual/{ja,en}/*.webp` +
>   `<img>` で載せている。却下理由 3 つがそちらには当たらないこと（連絡先が写らない /
>   ライト固定 + 節冒頭の断り / 要素クリップ + dsf 2 + lossy WebP で 1 枚 12〜32KB）は
>   あちらの ADR に個別に書いた
> - **したがって「図はすべて `assets/` の SVG」という読み方はもう成立しない。**
>   被写体で分かれる — **iOS のクロームは模式図 SVG、アプリ自身の画面は実スクショ**
