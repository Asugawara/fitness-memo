...
- **他アプリからの移行** — エクスポート機能を持たないアプリからでも移せる。前のアプリの画面をスクショし、iOS の「テキスト認識表示」で起こした文字を貼り付けると、日付・種目・セットを読み取って**足すだけ**で取り込む（[他アプリからの移行はスクショの文字起こしを貼り付けて受ける](adr/ux/migrate-by-ocr-paste.md)）。英語の種目名はプリセットの日本語名に寄せる。読み取れなかった行は全文を画面に出す。
...
- **他アプリから** — 前のアプリのスクショを iOS の「テキスト認識表示」で起こして貼り付ける。読み取った内容と、知らない種目の行き先（既存の部位）と、読み取れなかった行を出してから確認へ進む。**取り込みは常に「足すだけ」**（[テキスト取り込みは「足すだけ」に固定し、部位を増やさず、`at` を書かない](adr/data-model/text-import-is-merge-only.md)）
...
- 他アプリのデータをアプリ側から自動で読むことはできない（iOS のサンドボックス）。ショートカットも HealthKit も Web Share Target も経路にならないので、移行はスクショの文字起こしを貼り付ける形だけを持つ。狙いは**種目リストと直近の重量**で、数年分の履歴を再現する用途には応えない（[他アプリからの移行はスクショの文字起こしを貼り付けて受ける](adr/ux/migrate-by-ocr-paste.md)）。
