...
**リポジトリ設定でマージ方式を merge コミットのみに固定する。** 初期設定の手順に組み込む。
...
- **CLAUDE.md の規約だけでは担保されない。** このリポジトリには「Merge コミットを作成する / squash は避ける」という規約があるが、規約は**GitHub の Web UI でボタンを押すこと**を止めない。実際、設定を見に行くまでリポジトリは merge / squash / rebase の 3 つすべてが有効だった。GitHub の PR 画面はマージボタンにドロップダウンが付き、最後に使った方式が記憶される。1 回押し間違えると壊れる。
...
- **`gh api` の呼び出しが初期設定の必須手順になる。** 忘れると防御が効かない。
- 副作用として、`release` ブランチで `git log --oneline` を見ると `main` のコミットとデプロイコミットが混ざる。デプロイコミットは `deploy YYYY-MM-DD` という形式なので識別はできる。

## 検討した代替案

**規約（CLAUDE.md）だけで運用する**: 設定変更が不要。しかし規約は Web UI のボタンを止めないので、実質的に防御になっていない。規約を書いても 3 方式が有効なままだったことが、規約が設定に自動反映されないことの証拠である。却下。
