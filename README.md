...
自前のサーバもアカウント登録も持たず、記録は端末の `localStorage` にのみ保存する。**記録が外へ送られることは無い。**
（唯一の外向き通信は、利用者が Google スプレッドシートの URL を貼って「読み込む」を押したときの GET 1 本だけ。詳細は [公開リンクの Google スプレッドシートを URL で取り込む](adr/storage/import-from-published-sheet-url.md)）
...
タブは **記録 / 推移 / 種目** の 3 つ。記録タブはカレンダーと選択日の入力欄が縦に並んだ 1 画面で、日セルをタップするとその下の入力欄がその日のものになる（[記録タブをカレンダー + 選択日エディタの単一画面にする](adr/ux/record-tab-calendar-with-day-editor.md)）。
...
- **部位ごとのグループ分け** — 胸 / 背中 / 肩 / 腕 / 脚 / 体幹 の 6 部位に 28 種目をプリセットとして初期投入する。種目タブは**部位だけを並べ**、押すとその部位の種目が開く（同時に開くのは 1 つ）。名前と色の変更は右端の鉛筆から。部位も種目も追加・改名・削除ができる（[種目タブを部位の折りたたみ一覧にし、1 つだけ開く](adr/ux/menu-groups-as-single-open-accordion.md)）。
...
### 表計算（Excel / Numbers / Google スプレッドシート）

控え（JSON）とは別に、**1 行 = 1 セット の表**でも出し入れできる。ピボットで眺めたいときと、他アプリから移すときの経路。

- **表計算用に書き出す（CSV）** — UTF-8 BOM 付きなので、そのまま Excel / Numbers / スプレッドシートで開ける。iPhone では共有シートが開くので、Google ドライブに保存してスプレッドシートで開ける
- **スプレッドシートから読み込む** — シートを 共有 →「リンクを知っている全員」→「閲覧者」にして URL を貼る。スプレッドシートでセルを選んでコピーし、貼り付け欄に貼っても読める（この経路なら共有設定を変えなくてよい）

> [!NOTE]
> 表は **ID・部位の色・並び順を持たない**ので「置き換える」には使えず、**足すだけ**に固定される。丸ごとの復元は JSON のほうを使う。
> 未知の種目は `部位` 列に書いてある部位へ自動で作られ、部位が分からない行は**行番号と理由を出して**取り込まない（[表からの取り込みは利用者に何も聞かない](adr/ux/spreadsheet-import-asks-nothing.md)）。
