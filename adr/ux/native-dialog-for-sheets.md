...
**初期フォーカスを `<dialog>` へ引き取るのは、上の判断と矛盾しない。** 3 つ目は仕様どおりの挙動（dialog focusing steps）で両エンジンに共通するが、**Chromium はこの初期フォーカスを `:focus-visible` にマッチさせる**ため、指でタップして開いただけで ✕ に青枠が出る。利用者からは「何も押していないのに印が出た」と見える。復帰先の `focus()` を見送ったのは**利用者が押したコントロール**にリングを出さないためで、今回フォーカスを載せる先は `<dialog>` という**押せない器**であり、そこのリングは CSS で消してある。結論は両方とも「コントロールにリングを出さない」で同じ。

**フォーカスの位置を変えるだけではリングは消えない。** `:focus-visible` はプログラム的なフォーカス移動のとき「直前の要素がリングを出していたか」を引き継ぐので、`show_modal()` が ✕ をリング付きでフォーカスした直後に `focus()` すると `<dialog>` が引き継ぎうる。だから **Rust 側（フォーカスの位置）と CSS 側（リングを描かない）は対で要る**。`.sheet` は `width: 100%` なので、消さないと左右と下が画面端へ落ちて**上辺だけの横一文字の青線**になる。
