# ADR-0045: グラフの座標計算を `chart_layout` に切り出してテスト可能にする

- **状態**: 採用
- **日付**: 2026-08-09
- **カテゴリ**: architecture
- **関連**: [ADR-0004](0004-no-chart-library-hand-rolled-svg.md)（弱点の解消）, [ADR-0003](0003-wasm-target-scoped-dependencies.md), [ADR-0044](../ux/0044-body-weight-second-axis-always-on.md)

## 背景

[ADR-0004](0004-no-chart-library-hand-rolled-svg.md) は自前 SVG を選んだうえで、
結果の節に**自分で弱点を書いている**。
...
[ADR-0003](0003-wasm-target-scoped-dependencies.md) により `views` は `#[cfg(target_arch = "wasm32")]`
で gate されているので、`views/chart.rs` にある限りホストのテストからは触れない。

[ADR-0044](../ux/0044-body-weight-second-axis-always-on.md) で第2軸を足すにあたり、
...
**`core.rs` に移す（ADR-0004 の記述どおり）。** ファイルが増えない。しかし 2774 行にさらに
